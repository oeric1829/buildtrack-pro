...
const SK = { jobs:"bt_jobs", subs:"bt_subs", workers:"bt_workers" };
...
const CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .spinner { width:32px; height:32px; border:3px solid #252836; border-top-color:#F97316; border-radius:50%; animation:spin 0.8s linear infinite; }
...
export default function App() {
  ...
  async function submitCrewHours() {
    ...
    const sub = subs.find(s => s.id === session.subId);
    ...
    crew.forEach(w => {
      ...
      if (h > 0) {
        ...
        const entry = {
          ...
          jobId: selectedJob, jobName: job ? job.name : "",
          subId: session.subId, subName: sub ? sub.name : "",
        ...
        const ot = calcOT(entry, sub);
        const pay = calcPay(entry, sub);
        entry.pay = pay;
        ...
        entryRows.push({
          ...
          date: crewDate,
          hours: h,
          payType: sub ? sub.payType : "hourly",
          rate: sub ? sub.rate : 0,
          otHours: ot,
          pay: pay,
          approved: false,
    ...
    if (!newEntries.length) { showToast("Enter hours for at least one worker","error"); return; }

    setSyncing(true);
    setEntries(prev => [...prev, ...newEntries]);
    await apiPost({ action:"addEntries", entries:entryRows });
    setSyncing(false);
    setCrewHours({});
    setSubmitted(true);
    showToast(newEntries.length + " worker(s) submitted!");
  ...
  function exportCSV() {
    ...
    entries.forEach(e => {
      ...
      rows.push([
        s?s.name:e.subName||"", w?w.name:e.workerName||"", w?w.trade:"", j?j.name:e.jobName||"",
        fDate(e.clockIn), h.toFixed(2),
        s?(s.payType==="daily"?"$"+s.rate+"/day":"$"+s.rate+"/hr"):"",
        ot>0?ot.toFixed(2):"0", "$"+pay.toFixed(2),
  ...
  const totalCost = entries.reduce((a,e) => { const s=subs.find(x=>x.id===e.subId); return a+(s?calcPay(e,s):e.pay||0); }, 0);
  const totalHrs = entries.reduce((a,e) => a+calcHrs(e), 0);
  const pending = entries.filter(e => !e.approved).length;
  const jobStats = jobs.map(j => {
    const je = entries.filter(e => e.jobId===j.id || e.jobName===j.name);
    return { ...j, hrs:je.reduce((a,e)=>a+calcHrs(e),0), cost:je.reduce((a,e)=>{ const s=subs.find(x=>x.id===e.subId); return a+(s?calcPay(e,s):e.pay||0); },0) };
  });
  const subStats = subs.map(s => {
    const se = entries.filter(e => e.subId===s.id || e.subName===s.name);
    return { ...s, hrs:se.reduce((a,e)=>a+calcHrs(e),0), cost:se.reduce((a,e)=>a+calcPay(e,s),0) };
  }).filter(s => s.hrs > 0);
  const filteredEntries = entries.filter(e => {
    if (filterJob !== "all" && e.jobId !== filterJob && e.jobName !== jobs.find(j=>j.id===filterJob)?.name) return false;
    if (filterSub !== "all" && e.subId !== filterSub && e.subName !== subs.find(s=>s.id===filterSub)?.name) return false;
    return true;
  });
  ...
  return (
    <div style={{fontFamily:"'DM Sans','Segoe UI',sans-serif",minHeight:"100vh",background:"#0F1117",color:"#E8E9F0"}}>
      ...
      {toast && (
        <div className="fade" style={{position:"fixed",top:20,right:20,zIndex:500,background:toast.type==="error"?"#EF4444":toast.type==="info"?"#3B82F6":"#10B981",color:"#fff",padding:"13px 20px",borderRadius:12,fontWeight:600,fontSize:14,boxShadow:"0 10px 36px rgba(0,0,0,.5)"}}>
          {toast.msg}
        </div>
      )}

      {syncing && (
        <div style={{position:"fixed",bottom:20,right:20,zIndex:500,background:"#1A1D27",border:"1px solid #252836",borderRadius:12,padding:"10px 16px",display:"flex",alignItems:"center",gap:10,fontSize:13,color:"#8B8FA8"}}>
      ...
      {modal === "addJob" && (
        <Modal title={editing?"Edit Job":"Add New Job"} onClose={closeModal}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:4}}>
            {JOB_COLORS.map(c => <div key={c} onClick={() => setForm(p=>({...p,color:c}))} style={{width:28,height:28,borderRadius:"50%",background:c,cursor:"pointer",border:"3px solid "+(form.color===c?"#fff":"transparent")}} />)}
          </div>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"24px 20px"}}>
            ...
            {entries.filter(e => e.subId===session.subId||e.subName===subObj?.name).length > 0 && (
              <div className="card" style={{padding:20,marginTop:16}}>
                ...
                <table>
                  ...
                  <tbody>
                    {entries.filter(e=>e.subId===session.subId||e.subName===subObj?.name).slice().reverse().slice(0,8).map(e => {
                      const w = workers.find(x=>x.id===e.workerId);
                      const j = jobs.find(x=>x.id===e.jobId);
                      return (
                        <tr key={e.id}>
                          <td style={{fontWeight:600}}>{w?w.name:e.workerName||""}</td>
        ...
        {isAdmin && (
          <div className="fade">
            ...
            {loadingEntries && (
              <div style={{textAlign:"center",padding:"40px 0",color:"#8B8FA8",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",gap:12}}>
                <div className="spinner" /> Loading data from Google Sheets...
              </div>
            )}
              ...
              <>
                        ...
                        <table>
                          ...
                          <tbody>
                            {filteredEntries.slice().reverse().map(e => {
                              ...
                              const h = calcHrs(e);
                              const ot = calcOT(e,s);
                              const pay = calcPay(e,s)||e.pay||0;
                              return (
                                <tr key={e.id}>
                                  <td style={{fontWeight:600}}>{s?s.name:e.subName||""}</td>
                                  <td>{w?w.name:e.workerName||""}</td>
                                  <td><div style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:7,height:7,borderRadius:"50%",background:j?j.color:"#555"}} />{j?j.name:e.jobName||""}</div></td>
                ...
                {adminTab === "costs" && (
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(290px,1fr))",gap:14}}>
                    {jobStats.map(j => {
                      const sb = subs.map(s => {
                        const se = entries.filter(e=>e.jobId===j.id||e.jobName===j.name).filter(e=>e.subId===s.id||e.subName===s.name);
