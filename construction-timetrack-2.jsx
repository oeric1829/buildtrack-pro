...
// ─── DEFAULT DATA (loaded first time only) ────────────────────────────────────
const DEFAULT_JOBS = [
...
export default function App() {
  ...
  // Manage modals
  const [modal, setModal] = useState(null); // "addJob"|"editJob"|"addSub"|"editSub"|"addWorker"
  ...
  useEffect(()=>{
    async function load(){
      const [e,id,j,s,w]=await Promise.all([
        sGet(SK.entries,[]), sGet(SK.nextId,1),
        sGet(SK.jobs,null), sGet(SK.subs,null), sGet(SK.workers,null)
      ]);
      setEntries(e); setNextId(id);
      setJobs(j||DEFAULT_JOBS); setSubs(s||DEFAULT_SUBS); setWorkers(w||DEFAULT_WORKERS);
      setLoading(false);
    }
    load();
  ...
  useEffect(()=>{ if(!loading){ sSet(SK.entries,entries); } },[entries,loading]);
  useEffect(()=>{ if(!loading){ sSet(SK.nextId,nextId); } },[nextId,loading]);
  useEffect(()=>{ if(!loading){ sSet(SK.jobs,jobs); } },[jobs,loading]);
  useEffect(()=>{ if(!loading){ sSet(SK.subs,subs); } },[subs,loading]);
  useEffect(()=>{ if(!loading){ sSet(SK.workers,workers); } },[workers,loading]);
  ...
  function removeSub(id){
    if(!confirm("Remove this subcontractor? Their workers will also be removed.")) return;
  ...
  if(loading) return <div style={{minHeight:"100vh",background:"#0F1117",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"sans-serif",color:"#8B8FA8"}}>Loading...</div>;
  ...
  const CSS=`
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .btn-or{background:#F97316;color:#fff}.btn-or:hover{background:#EA6C10;transform:translateY(-1px);box-shadow:0 8px 24px rgba(249,115,22,.3)}
    .btn-or:disabled{background:#252836;color:#444;cursor:not-allowed;transform:none;box-shadow:none}
    .btn-gr{background:#10B981;color:#fff}.btn-gr:hover{background:#059669}
    .btn-rd{background:#EF4444;color:#fff}.btn-rd:hover{background:#DC2626}
    .btn-ghost{background:#252836;color:#E8E9F0;border:1px solid #33364A}.btn-ghost:hover{background:#2A2D3A}
    ...
    .tab.on{background:#F97316;color:#fff}.tab:hover:not(.on){color:#E8E9F0;background:#252836}
    ...
    .sc:hover{border-color:#F97316}.sc.on{border-color:#F97316;background:rgba(249,115,22,.1)}
    ...
    input,select,textarea{background:#252836;border:1px solid #33364A;border-radius:10px;color:#E8E9F0;padding:9px 13px;font-family:inherit;font-size:14px;outline:none;transition:border .15s;width:100%}
    ...
    td{padding:11px 11px;border-top:1px solid #1E2130}
    ...
    <div style={{fontFamily:"'DM Sans','Segoe UI',sans-serif",minHeight:"100vh",background:"#0F1117",color:"#E8E9F0"}}>
      ...
      {modal==="addJob"&&(
        <Modal title={editing?"Edit Job":"Add New Job"} onClose={closeModal}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:4}}>
            {JOB_COLORS.map(c=><div key={c} onClick={()=>setForm(p=>({...p,color:c}))} style={{width:28,height:28,borderRadius:"50%",background:c,cursor:"pointer",border:`3px solid ${form.color===c?"#fff":"transparent"}`,transition:"border .15s"}}/>)}
          </div>
      ...
      {modal==="addSub"&&(
        <Modal title={editing?"Edit Subcontractor":"Add Subcontractor"} onClose={closeModal}>
          ...
          <label>Rate ($)</label>
          <input type="number" placeholder="0" value={form.rate||""} onChange={e=>setForm(p=>({...p,rate:e.target.value}))} style={{width:"100%"}}/>
          <label>4-Digit PIN</label>
          <input type="number" placeholder="e.g. 7777" maxLength={4} value={form.pin||""} onChange={e=>setForm(p=>({...p,pin:e.target.value.slice(0,4)}))} style={{width:"100%"}}/>
        ...
        <div style={{maxWidth:1100,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:62}}>
          <div style={{display:"flex",alignItems:"center",gap:11}}>
            ...
            <div>
              ...
              <div style={{fontSize:11,color:"#8B8FA8"}}>{isAdmin?"👑 Admin":"👷 "+subObj?.name}</div>
            </div>
          </div>
        ...
        {!isAdmin&&(
          <div className="fade">
            <div style={{marginBottom:22}}>
              <h1 style={{fontFamily:"'Space Grotesk',sans-serif",fontSize:21,fontWeight:700}}>Daily Hours — {subObj?.name}</h1>
              ...
              <div className="card fade" style={{padding:22}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18,flexWrap:"wrap",gap:10}}>
                  <div>
                    ...
                    <div style={{fontWeight:700,fontSize:15}}><span style={{color:"#F97316"}}>{jobs.find(j=>j.id===selectedJob)?.name}</span></div>
                  </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))",gap:11,marginBottom:20}}>
                  {crewWorkers.map(w=>{ const h=parseFloat(crewHours[w.id]||0),active=h>0; return(
                    <div key={w.id} style={{background:"#1E2130",borderRadius:12,padding:15,border:`2px solid ${active?"#F97316":"transparent"}`,transition:"border .2s"}}>
                      <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:11}}>
                        <div style={{width:30,height:30,borderRadius:"50%",background:active?"#F97316":"#252836",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:"#fff",flexShrink:0,transition:"background .2s"}}>{ini(w.name)}</div>
            ...
            {entries.filter(e=>e.subId===session.subId).length>0&&(
              <div className="card" style={{padding:20,marginTop:16}}>
                ...
                <tbody>{entries.filter(e=>e.subId===session.subId).slice().reverse().slice(0,8).map(e=>{ const w=workers.find(x=>x.id===e.workerId),j=jobs.find(x=>x.id===e.jobId),h=calcHrs(e); return(
                  <tr key={e.id}>
                    <td style={{fontWeight:600}}>{w?.name}</td>
                    <td><div style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:7,height:7,borderRadius:"50%",background:j?.color}}/>{j?.name}</div></td>
        ...
        {isAdmin&&(
          <div className="fade">
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:13,marginBottom:20}}>
              {[{label:"Total Entries",value:entries.length,icon:"📋",color:"#3B82F6"},{label:"Total Hours",value:totalHrs.toFixed(1)+"h",icon:"⏱️",color:"#F97316"},{label:"Pending",value:pending,icon:"⚠️",color:"#FBBF24"},{label:"Est. Labor Cost",value:"$"+totalCost.toLocaleString("en-US",{maximumFractionDigits:0}),icon:"💰",color:"#10B981"}].map(s=>(
            ...
            {adminTab==="overview"&&(
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
                <div className="card" style={{padding:20}}>
                  ...
                  {!subStats.length&&<div style={{color:"#444",fontSize:13}}>No entries yet.</div>}
                  {subStats.map(s=>{ const pct=totalHrs>0?(s.hrs/totalHrs)*100:0; return(
                    <div key={s.id} style={{marginBottom:13}}>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                        <div><span style={{fontSize:13,fontWeight:600}}>{s.name}</span><span style={{fontSize:11,color:"#555",marginLeft:5}}>{s.payType==="daily"?`$${s.rate}/day`:`$${s.rate}/hr`}</span></div>
                        <span style={{fontSize:13,fontWeight:700}}><span style={{color:"#F97316"}}>{s.hrs.toFixed(1)}h</span><span style={{color:"#444",margin:"0 3px"}}>·</span><span style={{color:"#10B981"}}>${s.cost.toFixed(0)}</span></span>
                      </div>
                      <div style={{height:4,background:"#252836",borderRadius:2}}><div style={{height:"100%",width:`${pct}%`,background:"#F97316",borderRadius:2}}/></div>
                    </div>
                  ); })}
                </div>
                <div className="card" style={{padding:20}}>
                  <div style={{fontWeight:700,fontSize:14,marginBottom:14}}>📍 By Job Site</div>
                  {jobStats.map(j=>{ const pct=totalHrs>0?(j.hrs/totalHrs)*100:0; return(
                    <div key={j.id} style={{marginBottom:13}}>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                        <div style={{display:"flex",alignItems:"center",gap:7}}><div style={{width:8,height:8,borderRadius:"50%",background:j.color}}/><span style={{fontSize:13,fontWeight:600}}>{j.name}</span></div>
                        <span style={{fontSize:13,fontWeight:700}}><span style={{color:j.color}}>{j.hrs.toFixed(1)}h</span><span style={{color:"#444",margin:"0 3px"}}>·</span><span style={{color:"#10B981"}}>${j.cost.toFixed(0)}</span></span>
                      </div>
                      <div style={{height:4,background:"#252836",borderRadius:2}}><div style={{height:"100%",width:`${pct}%`,background:j.color,borderRadius:2}}/></div>
                    </div>
                  ); })}
                  {!totalHrs&&<div style={{color:"#444",fontSize:13}}>No entries yet.</div>}
                </div>
              </div>
            )}
            ...
            {adminTab==="timesheets"&&(
              <div className="card" style={{padding:20}}>
                <div style={{display:"flex",gap:9,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
                  ...
                  {entries.some(e=>!e.approved)&&<button className="btn btn-gr btn-sm" onClick={approveAll} style={{marginLeft:"auto"}}>✓ Approve All</button>}
                </div>
                {!filteredEntries.length?<div style={{textAlign:"center",color:"#444",padding:"36px 0",fontSize:13}}>No entries yet.</div>:(
                  <div style={{overflowX:"auto"}}>
                    <table><thead><tr>{["Sub","Worker","Job","Date","Hrs","Rate","OT","Pay","Status",""].map(h=><th key={h}>{h}</th>)}</tr></thead>
                    <tbody>{filteredEntries.slice().reverse().map(e=>{ const w=workers.find(x=>x.id===e.workerId),s=subs.find(x=>x.id===e.subId),j=jobs.find(x=>x.id===e.jobId),h=calcHrs(e),ot=calcOT(e,s),pay=calcPay(e,s); return(
                      <tr key={e.id}>
                        <td style={{fontWeight:600}}>{s?.name}</td><td>{w?.name}</td>
                        <td><div style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:7,height:7,borderRadius:"50%",background:j?.color}}/>{j?.name}</div></td>
                        <td style={{color:"#8B8FA8",whiteSpace:"nowrap"}}>{fDate(e.clockIn)}</td>
                        <td style={{fontWeight:700,color:"#F97316"}}>{h.toFixed(1)}h</td>
                        <td style={{color:"#8B8FA8",fontSize:11}}>{s?.payType==="daily"?`$${s.rate}/day`:`$${s.rate}/hr`}</td>
                        <td style={{color:ot>0?"#FBBF24":"#444",fontWeight:ot>0?700:400}}>{s?.payType==="daily"?"—":ot>0?`+${ot.toFixed(1)}h`:"—"}</td>
                        <td style={{color:"#10B981",fontWeight:700}}>${pay.toFixed(0)}</td>
                        <td><span className="pill" style={{background:e.approved?"rgba(16,185,129,.14)":"rgba(249,115,22,.14)",color:e.approved?"#10B981":"#F97316"}}>{e.approved?"✓":"Pending"}</span></td>
                        <td><div style={{display:"flex",gap:4}}>{!e.approved&&<button className="btn btn-gr btn-sm" onClick={()=>approveEntry(e.id)}>✓</button>}<button className="btn btn-rd btn-sm" onClick={()=>deleteEntry(e.id)}>✕</button></div></td>
                      </tr>); })}
                    </tbody></table>
                  </div>
                )}
            ...
            {adminTab==="costs"&&(
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(290px,1fr))",gap:14}}>
                {jobStats.map(j=>{ const sb=subs.map(s=>{ const se=entries.filter(e=>e.jobId===j.id&&e.subId===s.id); if(!se.length) return null; return {...s,hrs:se.reduce((a,e)=>a+calcHrs(e),0),cost:se.reduce((a,e)=>a+calcPay(e,s),0)}; }).filter(Boolean); return(
                  <div className="card" key={j.id} style={{padding:20}}>
                    <div style={{display:"flex",alignItems:"center",gap:9,marginBottom:16}}><div style={{width:10,height:10,borderRadius:"50%",background:j.color}}/><div style={{fontWeight:700,fontSize:15}}>{j.name}</div></div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,marginBottom:14}}>
                      <div style={{background:"#252836",borderRadius:9,padding:12}}><div style={{fontSize:19,fontWeight:700,color:j.color,fontFamily:"'Space Grotesk',sans-serif"}}>{j.hrs.toFixed(1)}h</div><div style={{fontSize:10,color:"#8B8FA8",marginTop:2}}>Total Hours</div></div>
                      <div style={{background:"#252836",borderRadius:9,padding:12}}><div style={{fontSize:19,fontWeight:700,color:"#10B981",fontFamily:"'Space Grotesk',sans-serif"}}>${j.cost.toFixed(0)}</div><div style={{fontSize:10,color:"#8B8FA8",marginTop:2}}>Labor Cost</div></div>
                    </div>
                    {sb.length>0&&<>{<div style={{fontSize:10,color:"#8B8FA8",fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:7}}>By Sub</div>}{sb.map(s=><div key={s.id} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:"1px solid #1E2130",fontSize:12}}><div><div style={{fontWeight:500}}>{s.name}</div><div style={{fontSize:10,color:"#555"}}>{s.payType==="daily"?`$${s.rate}/day`:`$${s.rate}/hr`}</div></div><div style={{textAlign:"right"}}><div style={{color:"#F97316",fontWeight:600}}>{s.hrs.toFixed(1)}h</div><div style={{color:"#10B981",fontWeight:700}}>${s.cost.toFixed(0)}</div></div></div>)}</>}
                    {!j.hrs&&<div style={{color:"#444",fontSize:12}}>No hours yet.</div>}
                  </div>
                ); })}
              </div>
            )}
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
                    ...
                    <div key={s.id} style={{padding:"9px 0",borderBottom:"1px solid #1E2130"}}>
                      <div style={{display:"flex",alignItems:"center",gap:9}}>
                        ...
                        <div style={{flex:1}}>
                          ...
                          <div style={{fontSize:11,color:"#8B8FA8"}}>{s.trade} · <span style={{color:"#10B981"}}>{s.payType==="daily"?`$${s.rate}/day`:`$${s.rate}/hr`}</span> · PIN: <span style={{color:"#F97316",fontWeight:700}}>{s.pin}</span></div>
                        </div>
                ...
                <div className="card" style={{padding:20,gridColumn:"1/-1"}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:8}}>
                    {subs.map(s=>{ const sw=workers.filter(w=>w.subId===s.id); if(!sw.length) return null; return(
